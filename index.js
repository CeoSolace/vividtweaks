...
async function createOrGetTicketChannel(guild, userId) {
  // reuse an open ticket if it still exists
...
async function executeRefundInternal(purchase) {
  ...
  if (purchase.stripePaymentIntentId) {
    ...
    // cancel subscription immediately on refund attempt
...
async function registerCommands() {
  ...
  // Guild commands only (fast updates, and you're locked to one guild anyway)
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      ...
      // Ticket plan buttons -> generate checkout URL and post in ticket
      if (interaction.customId.startsWith("tp:")) {
        ...
        // Ensure this channel is an open ticket and caller is ticket owner
...
// IMPORTANT: raw body only on webhook route
app.post("/stripe/webhook", bodyParser.raw({ type: "application/json" }), async (req, res) => {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      // Stripe IDs stored ONLY in DB (never posted to Discord)
    ...
    // Remove role when Stripe says the subscription is actually over
